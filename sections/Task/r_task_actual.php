...
if (strlen($func) == 0) {
	...
	$response = r_Task_Actual_DrawReport($OwnerID);
	echo $response;
}
else {
	//если передаем POST
...
function r_Task_Actual_DrawReport($p_OwnerID)
{
	$con = GetConnection();
	
	//Сформируем отчет
	$result = LoadTemplate('r_task_actual.html');
	...
	$p_EndDate = date('d.m.Y', strtotime($Date)+$diff*8); 

	//Получим перечень дел
	$select_sql = r_Task_GetTaskSelect()
			. "where t.OwnerID = :p_OwnerID " 
			. "and ts.code in ('Active', 'Plan', 'Future') "
			. "and (t.FinishDate < :p_FinishDate or t.FinishDate is null) "
			. "order by t.StartDate asc";
	$statement = $con->prepare($select_sql);
	$statement->execute(array(
		':p_OwnerID' => $p_OwnerID,
		':p_FinishDate' => $p_EndDate
	));
	...
	foreach ($res as $row) {
		...
		if (strtotime($row['finishdate'])<strtotime($Date)) {
//			$col_class = 'softred';		
			$row_class = 'softred';		
		}
		...
		if ($row['contact'] != '') {
			$result .= '<br/>Контакт: '.$row['contact']; 
		}
		if ($row['project'] != '') {
			$result .= '<br/>Проект: '.$row['project']; 
		}
		if ($row['product'] != '') {
			$result .= '<br/>Продукт: '.$row['product'];
		...
		$result .= '<table width="100%"><tbody><tr class="'.$row_class.'">'; 
		$result .= '<td class="normal">Важность:</td>'; 
		$result .= '<td class="normal">'.$row['importance'].'</td>'; 
		$result .= '</tr>'; 
		$result .= '<tr>'; 
		$result .= '<td class="normal">Состояние:</td>'; 
		$result .= '<td class="normal">'.$row['state'].'</td>'; 
		$result .= '</tr>'; 
		$result .= '<tr>'; 
		$result .= '<td class="normal">Тип:</td>';
		$result .= '<td class="normal">'.$row['tasktype'].'</td>';
		$result .= '</tr>'; 
		if ($row['tasktypecode'] == 'Meet') {
			$result .= '<tr>'; 
			$result .= '<td class="normal" colspan="2">'.$row['city'].', '.$row['address'].'</td>'; 
			$result .= '</tr>'; 
		}
		if (($row['tasktypecode'] == 'Meet')||($row['tasktypecode'] == 'Call')) {
			$result .= '<tr>'; 
			$result .= '<td class="normal" colspan="2">'.$row['aphone1'].'; '.$row['aphone2'].'; '.$row['aphone3'].'</td>'; 
			$result .= '</tr>'; 
		}
		$result .= '</tbody></table>'; 
		$result .= '</tr>'; 
